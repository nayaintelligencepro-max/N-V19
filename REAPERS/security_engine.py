...
class IdentityGuard:
    ...
    def create_session(self, user_id: str) -> str:
        ...
        session_id = hashlib.sha256(f"{user_id}_{datetime.now(timezone.utc)}".encode()).hexdigest()
...
class EncryptionManager:
    """
    Manages data masking for sensitive fields.

    NOTE: Full AES-256 encryption requires the `cryptography` package
    (pip install cryptography) and a persisted ENCRYPTION_KEY env var.
    When unavailable, we fall back to deterministic HMAC-SHA256 masking
    (one-way — irreversible) which is sufficient for audit logs but NOT
    for round-trip encryption. Set ENCRYPTION_KEY to enable real encryption.
    """

    def __init__(self):
        import os
        self._key = os.environ.get("ENCRYPTION_KEY", "").encode() or None
        self._fernet = None
        if self._key:
            try:
    ...
    def encrypt(self, data: str) -> str:
        """Encrypt data. Uses Fernet (AES-128-CBC) if key available, else HMAC mask."""
        if self._fernet:
            return self._fernet.encrypt(data.encode()).decode()
        # Fallback: HMAC-SHA256 masking (one-way, audit use only)
        return "MASKED:" + hashlib.sha256(data.encode()).hexdigest()

    def decrypt(self, encrypted: str) -> str:
        """Decrypt data. Returns original only if Fernet key is set and data was encrypted with it."""
        if self._fernet and not encrypted.startswith("MASKED:"):
            try:
                return self._fernet.decrypt(encrypted.encode()).decode()
            except Exception:
                pass
        # Cannot reverse a masked/hashed value
        return encrypted
