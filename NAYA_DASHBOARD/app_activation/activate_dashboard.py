...
class ActivateDashboard:
    """Activate Dashboard — Production implementation."""
    ...
    def activate(self, *args, **kwargs) -> Any:
        ...
        with self._lock:
            ...
            t0 = time.time()
            try:
                result = {"status": "ok", "op": "activate", "ts": time.time()}
                elapsed = (time.time() - t0) * 1000
                self._history.append({"op": "activate", "ts": time.time(), "ms": round(elapsed, 1)})
                ...
                self._errors += 1
                log.error(f"[ActivateDashboard] activate: {e}")
                return None
    ...
    def get_stats(self) -> Dict:
        with self._lock:
            return {
                "class": "ActivateDashboard", "active": self._active, "healthy": self.is_healthy(),
